...
public class SimAnnealing {
    ...
    private static String name = "SimAnneal_2";

    public static void main(String[] args) {
        // Args
        if (args.length != 2) {
            System.out.println("Usage: java SimAnnealing <instance>");
            return;
        }
        ...
        SolutionSaver solutionSaver = new SolutionSaver("w10_own", name, instance);
        // Perform experiment
    ...
    private static List<Integer> generateHEA(List<List<Integer>> initialpop, List<Node> nodeList, int[][] distanceMatrix, long startTime, int operator, int maxTime) {
        ...
        while (System.currentTimeMillis() - startingTime < maxTime) {
            ...
            if (RANDOM.nextDouble()<0.5){
                ...
                child = simulatedAnnealing(child, nodeList, distanceMatrix, 10000, 0.995, 1);
            }
    ...
    public static List<Integer> simulatedAnnealing(List<Integer> initialSolution, List<Node> nodeList, int[][] distanceMatrix, double T, double coolingRate, double stoppingTemperature
    ) {
        List<Integer> currentSolution = new ArrayList<>(initialSolution);
        double currentCost = evaluate(currentSolution, nodeList, distanceMatrix);
        ...
        while (T > stoppingTemperature) {
            ...
            double neighborCost = evaluate(neighbor, nodeList, distanceMatrix);
    ...
    public static List<Integer> randomInsertion(List<Integer> solution, int n) {
        for (int i = 0; i < n; i++) {
            int which = RANDOM.nextInt(solution.size());
            int where = RANDOM.nextInt(solution.size() + 1);
            solution.add(where, solution.remove(which));
        }
        return solution;
    }
    ...
    public static List<Integer> kOptMove(List<Integer> solution, int k) {
        int n = solution.size();
        List<Integer> edges = RANDOM.ints(0, n).distinct().limit(k).sorted().boxed().toList();
        ...
        for (int i = 0; i < k; i++) {
            ...
            if (start < end) {
                newSolution.addAll(solution.subList(start, end));
            } else {
                newSolution.addAll(solution.subList(start, n));
                newSolution.addAll(solution.subList(0, end));
            }

            if (i % 2 == 1) {
                Collections.reverse(newSolution.subList(newSolution.size() - (end - start), newSolution.size()));
            }
        }

...
public class MultipleStartLocalSearch {
    ...
    public static void main(String[] args) {
        // Args
        if (args.length != 2) {
            System.out.println("Usage: java MultipleStartLocalSearch <instance> <n_experiments>");
            return;
        }
        ...
        SolutionSaver solutionSaver = new SolutionSaver("w6_msls_ils", "MultipleStartLocalSearch", instance);
        ...
        for (int i = 1; i <= nExperiments; i++) {
            ...
            List<Integer> randomSolution = generateMultipleStartLocalSearch(nodeList, distanceMatrix, ids);
    ...
    private static List<Integer> generateMultipleStartLocalSearch(List<Node> nodeList, int[][] distanceMatrix, List<Integer> ids) {
        int totalNodes = nodeList.size();
        int numberToSelect = totalNodes / 2; // Select 50%

        List<Integer> bestSolution = new ArrayList<>();
        int lowestObjectiveFunction = Integer.MAX_VALUE;
        int count = 0;
        ...
        while(count<200){
            ...
            SortedSet<Move> LM = new TreeSet<>(Comparator
                    ...
                    .thenComparing(Move::getNextNode)); // & then by neighbor
            ...
            while (go) {
                ...
                // for moves m from LM starting from the best until an applicable move is found
            ...
            int totalDistance = evaluator.calculateTotalDistance(selectedIds, distanceMatrix);
            int objFuncValue = evaluator.calculateObjectiveFunction(totalCost, totalDistance);
            if (objFuncValue<lowestObjectiveFunction){
                lowestObjectiveFunction = objFuncValue;
                bestSolution = selectedIds;
            }
            count = count+1;
        }
        return bestSolution;
